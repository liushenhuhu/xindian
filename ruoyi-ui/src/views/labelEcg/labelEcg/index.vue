  ...
  <div class="app-container">
      ...
      <div class="table-hand">
        <div class="table-hand-left">
          ...
          <el-select
            ...
            clearable
            @change="getList"
            class="table-hand-left-select table-hand-left-select-status"
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span>{{
                parseTime(scope.row.connectionTime, "{y}-{m}-{d} {h}:{i}:{s}")
              }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="风险等级" align="center" prop="ecgLevel">
            <template slot-scope="scope">
              <dict-tag
                :options="dict.type.ecg_level"
                :value="scope.row.ecgLevel"
              />
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              <dict-tag
                :options="dict.type.diagnosis_status"
                :value="scope.row.diagnosisStatus"
              />
            </template>
          </el-table-column>
          <el-table-column
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="智能诊断" prop="intelligentDiagnosis">
          <el-input
            v-model="form.intelligentDiagnosis"
            placeholder="请输入智能诊断"
          />
        </el-form-item>
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      if (this.verifyForm.status || isShowName) {
        ...
        this.$modal
          .confirm('是否确认删除用户管理编号为"' + pIds + '"的数据项？')
          .then(function () {
    ...
    handleExport() {
      ...
      if (this.verifyForm.status || isShowName) {
        this.download(
          "patient_management/patient_management/export",
          {
            ...this.queryParams,
          },
    ...
    lookHistoryData(val) {
      ...
      if (daolian == "JECGs") {
        this.$router.push({
          path: "/historyData",
          query: { patientPhone: val.patientPhone, ecgType: "JECGsingle" },
        });
      } else if (daolian == "JECG1") {
        this.$router.push({
          path: "/historyData",
          query: { patientPhone: val.patientPhone, ecgType: "JECG12" },
        });
      } else if (daolian == "JECG4") {
        this.$router.push({
          path: "/historyData",
          query: { patientPhone: val.patientPhone, ecgType: "JECG4" },
        });
      } else {
    ...
    handleInform(row) {
      let routeUrl = this.$router.resolve({
        path: "/restingECG",
        query: { pId: row.pId, hospitalName: row.hospitalName },
      });
    ...
    downloadData(row) {
      ...
      if (dizhi == "JECGs") {
        window.open(
          `https://ecg.mindyard.cn:84/DECGReport/ECG_single/save/${row.pId}.dat`
        );
      } else if (dizhi == "JECG1") {
        window.open(
          `https://ecg.mindyard.cn:84/DECGReport/ECG/save/${row.pId}.dat`
        );
      } else if (dizhi == "JECG4") {
        window.open(
          `https://ecg.mindyard.cn:84/DECGReport/ECG4Lead/save/${row.pId}.dat`
        );
      } else {
        // this.$modal.msgError("没有此类型");
        this.$modal.msgSuccess("没有此类型");
