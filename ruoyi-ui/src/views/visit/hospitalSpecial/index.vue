...
<!--        </el-form-item>-->
        <el-form-item label="专科名称" prop="specialId">
          <el-select v-model="form.specialId" placeholder="请输入专科名称" >
            <el-option
              v-for="item in specialList"
              :key="item.hospitalId"
              :label="item.specialName"
...
<script>
import { listRelation, getRelation, delRelation, addRelation, updateRelation } from "@/api/visit/hospitalRelation";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        specialId: [
          { required: true, message: "专科编号不能为空", trigger: "blur" }
        ],
        gmtCreate: [
  ...
  methods: {
    ...
    findOutpatient(row){
      this.$router.push({path: "/visit/hospitalOutpatient" , query: {specialId: row.specialId,hospitalId: row.hospitalId}});
    },
    /** 搜索按钮操作 */
    handleQuery() {
    ...
    handleAdd() {
      this.reset();
      let _th = this
      _th.specialList.forEach(function(value) {
        value.disabled=false
        _th.relationList.forEach(function(a) {
          if (value.specialId===a.specialId){
            value.disabled=true
          }
        });
      });
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除医院专科编号为"' + ids + '"的数据项？').then(function() {
