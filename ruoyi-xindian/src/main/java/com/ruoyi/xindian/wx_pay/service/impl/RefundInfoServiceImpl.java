...
import com.ruoyi.xindian.wx_pay.mapper.RefundInfoMapper;
...
public class RefundInfoServiceImpl extends ServiceImpl<RefundInfoMapper, RefundInfo> implements RefundInfoService {
     ...
     */
    @Override
    public void updateRefund(String xmlStr) throws Exception {

        Map<String, Object> resultMap = WXPayUtil.xmlToMap(xmlStr);
        ...
        if(resultMap.get("status") != null){
            ...
            refundInfo.setContentReturn(xmlStr);//将全部响应结果存入数据库的content字段
        }
        //退款回调中的回调参数
        if(resultMap.get("refund_status") != null){
            refundInfo.setRefundStatus((String) resultMap.get("refund_status"));//退款状态
            refundInfo.setContentNotify(xmlStr);//将全部响应结果存入数据库的content字段
        }
