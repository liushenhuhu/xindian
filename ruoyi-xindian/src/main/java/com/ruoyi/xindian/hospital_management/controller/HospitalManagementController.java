package com.ruoyi.xindian.hospital_management.controller;
 ...
 */
@RestController
@RequestMapping("/hospital_management/hospital_manangment")
public class HospitalManagementController extends BaseController
{
     ...
     * 查询医院管理列表
     */
    @PreAuthorize("@ss.hasPermi('hospital_management:hospital_manangment:list')")
    @GetMapping("/list")
     ...
     * 导出医院管理列表
     */
    @PreAuthorize("@ss.hasPermi('hospital_management:hospital_manangment:export')")
     ...
     * 获取医院管理详细信息
     */
    @PreAuthorize("@ss.hasPermi('hospital_management:hospital_manangment:query')")
    @GetMapping(value = "/{id}")
    public AjaxResult getInfo(@PathVariable("id") Long id)
    {
        return AjaxResult.success(hospitalManagementService.selectHospitalManagementById(id));
    }

    /**
     * 新增医院管理
     */
    @PreAuthorize("@ss.hasPermi('hospital_management:hospital_manangment:add')")
     ...
     * 修改医院管理
     */
    @PreAuthorize("@ss.hasPermi('hospital_management:hospital_manangment:edit')")
     ...
     * 删除医院管理
     */
    @PreAuthorize("@ss.hasPermi('hospital_management:hospital_manangment:remove')")
    @Log(title = "医院管理", businessType = BusinessType.DELETE)
	@DeleteMapping("/{ids}")
    public AjaxResult remove(@PathVariable Long[] ids)
    {
        return toAjax(hospitalManagementService.deleteHospitalManagementByIds(ids));
    }
}
