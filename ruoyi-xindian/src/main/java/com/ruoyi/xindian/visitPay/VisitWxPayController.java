package com.ruoyi.xindian.visitPay;
 ...
 * @Description 门诊预约微信支付相关接口
 * @createTime 2023年07月09日 13:09:00
 */
@RestController
@RequestMapping(value = "/visit/pay")
public class VisitWxPayController {
...
//        LoginUser loginUser = tokenService.getLoginUser(request);
        ...
        try {
            ...
            //查询订单信息
            VisitAppointment visitAppointment = visitAppointmentService.selectVisitAppointmentById(orderId);
            if (visitAppointment==null){
                throw new ServiceException("预约门诊不存在");
            }
            if(!visitAppointment.getPayStatus().equals("0")){
                throw new ServiceException("预约门诊已支付");
            }
            String body = visitAppointment.getTitle();//订单介绍
            String orderNum = visitAppointment.getOrderNo();//商户订单号，由随机数组成
            String openId = visitAppointment.getOpenId();//获取到用户登录的openid
            BigDecimal price = visitAppointment.getPayPrice();//价格
...
//       System.out.println("body= "+body);
            ...
            paraMap.put("notify_url",WXPayConstants.VISIT_CALLBACK_URL);// 此路径是微信服务器调用支付结果通知路径
...
//       String xml = new String(WXPayUtil.mapToXml(paraMap).getBytes(), "utf-8");
    ...
    public Map<String, Object> refund(String id,HttpServletResponse response){
        ...
        try {
            ...
            String orderNum = visitAppointment.getOrderNo();//商户订单号，由随机数组成
            BigDecimal price = visitAppointment.getPayPrice();//金额

            //创建退款记录
            RefundInfo refundByOrderNo = refundsInfoService.createRefundByVisitOrderNo(orderNum, reason);
//       Integer price = 1;//支付金额，单位：分，这边需要转成字符串类型，否则后面的签名会失败
...
//       paraMap.put("notify_url", WXPayConstants.notify_url);   //退款路径
            ...
            if (xmlStr.indexOf("SUCCESS") != -1) {
                ...
                if(map.get("return_code").equals("SUCCESS")){
                    ...
                    try {
                        //修改订单状态
                        visitAppointmentService.updateVisitAppointmentOrderStatus(orderNum,"2");
                        visitAppointmentService.updateVisitAppointmentStatus(id,"2");
    ...
    public Boolean orderQuery(String id){
        ...
        try {
            ...
            if (map.get("result_code").equals("SUCCESS")&&map.get("return_code").equals("SUCCESS")&&map.get("trade_state").equals("SUCCESS")){
                wxPayService.visitPay(xmlStr);
                return true;
            }
            redisTemplate.opsForValue().set("orderVisitQuery:"+id,id,20, TimeUnit.SECONDS);
            return false;
